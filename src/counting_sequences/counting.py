...
from mbf.align import Sample, fastq
...
from .util import (
    count_raw_input_reads,
    get_sequence_df_from_excel_file,
    identity,
)
...
class SequenceCounter:
    ...
    def combine(
        ...
        def dump(outfile, inputfiles=inputfiles):
            concats = []
        ...
        return FileGeneratingJob(outfile, dump).depends_on(dependencies)
    ...
    def get_dependencies(self) -> List[Job]:
        ...
        return self.dependencies + [
            FileInvariant(self.sequence_file_path),
            FunctionInvariant(
    ...
    def __dump_predefined_sequences(self, outputfile):
        ...
        try:
            pd.testing.assert_frame_equal(
                df_sequence_df[["Duplicate"]],
                df_sequence_df[["Duplicate Full"]],
            )
    ...
    def write_predefined_sequences(self) -> Job:
        ...
        return FileGeneratingJob(
            outputfile, self.__dump_predefined_sequences
        ).depends_on(self.get_dependencies())

    def _write_fastq_count_callable(self, raw_lane: Sample):
        """
        Callable for write_fastq_count counts all occuring sequences in a fastq file.
        ...
        -------
        Callable
            A function that generates the sequence count table.
        """

        def __write(output_file, self=self, raw_lane=raw_lane):
    ...
    def write_fastq_count(
        self, raw_lane: Sample, dependencies: List[Job] = []
    ) -> FileGeneratingJob:
        """
        ...
        -------
        FileGeneratingJob
            A job that generates the sequence count table file.
        """
        output_file = (
        ...
        return (
            FileGeneratingJob(
                output_file,
                self._write_fastq_count_callable(raw_lane),
                empty_ok=True,
            )
    ...
    def count_fastq(self, raw_lane: Sample) -> DataFrame:
        ...
        for seq1, _, _ in fastq.iterate_fastq(str(fn1), False):
            seq1 = seq1.decode()
            counter[seq1] += 1
        for seqs in counter:
    ...
    def _write_count_table_callable(
        self, raw_lane: Sample, row_order: Optional[List[str]] = None
    ):
        """
        ...
        -------
        Callable
            Callable that creates the output table.
        """

        def __write(
            output_files, self=self, raw_lane=raw_lane, row_order=row_order
        ):
    ...
    def write_count_table(
        ...
        return (
            MultiFileGeneratingJob(
                ...
                self._write_count_table_callable(raw_lane, row_order),
            )
            .depends_on(
    ...
    def write_fastq_count_trimmed_callable(self, raw_lane: Sample):
        """
        ...
        -------
        Callable
            Creates a DataFrame containing the trimmed sequences and corresponding counts.
        """

        def __write(output_file, raw_lane=raw_lane, self=self):
            read_counter_file = (
                self.result_dir / f"{raw_lane.name}_{self.name}_all_reads.tsv"
            )
    ...
    def write_fastq_count_trimmed(self, raw_lane: Sample) -> Job:
        ...
        return (
            FileGeneratingJob(
                output_file, self.write_fastq_count_trimmed_callable(raw_lane)
            )
            .depends_on(self.write_fastq_count(raw_lane))
            .depends_on(self.dependencies)
            .depends_on(raw_lane.prepare_input())
    ...
    def _generate_fastq_from_unmatched_callable(
        self, raw_lane: Sample
    ) -> Callable:
        """
        ...
        -------
        Callable
            The function generating the fastq.
        """
        infile = (
            self.result_dir
            / f"{raw_lane.name}_{self.name}_sequence_count_unmatched.tsv"
        ...
        def __dump_fastq(outfile, infile=infile):
    ...
    def generate_fastq_from_unmatched(
        self, raw_lane: Sample
    ) -> FileGeneratingJob:
        """
        Generates a fastq for the unmatched files.
        ...
        return (
            FileGeneratingJob(
                outfile, self._generate_fastq_from_unmatched_callable(raw_lane)
            )
...
def grep_unmatched_reads_callable(
    outfile: Path, raw_lane: Sample, df_file: DataFrame
) -> Callable:
    """
    grep_unmatched_reads_callable iterates through a fastq and compares the
    sequences to a set of predefined sequences from a DataFrame, then writes a
    fastq file for the sequences not contained in the DataFrame.

    Parameters
    ----------
    outfile : Path
        The path to the newly generated fastq
    raw_lane : Sample
        The sample to get the inpuit fastq from
    df_file : DataFrame
        DataFrame with predefined sequences

    Returns
    -------
    Callable
        A function that creates the new fastq
    """

    def __dump(outfile, raw_lane=raw_lane, df_file=df_file):
        outfile.parent.mkdir(parents=True, exist_ok=True)
        ...
        with outfile.open("w") as output:
            for seq1, qual1, name1 in fastq.iterate_fastq(
                str(full_fasta_file), False
            ):
                seq1, qual1, name1 = (
                    seq1.decode(),
                    qual1.decode(),
...
def grep_unmatched_reads(
    ...
    -------
    Job
        A job that creates the new fastq
    """

    return FileGeneratingJob(
        outfile, grep_unmatched_reads_callable(raw_lane, df_file)
    ).depends_on(dependencies)
...
class SequenceCounter2(SequenceCounter):
    ...
    def _write_predefined_sequences_callable(self) -> Callable:
        """
        write_predefined_sequences returns a job that creates a table
        of predefined sequences to search for.
        ...
        def __dump(outputfile, self=self):
            df_sequence_df = self.get_sequence_df()
            df_sequence_df[self.id_column] = df_sequence_df[
                self.id_column
            ].astype(str)
            if self.sequence_df_filter is not None:
                df_sequence_df = self.sequence_df_filter(df_sequence_df)
            ...
            if trim_function is not None:
                # rename the original sequnence and use the trimmed ones
                df_sequence_df = df_sequence_df.rename(
                    columns={self.sequence_column: "Full Sequence"}
                )
                df_sequence_df[self.sequence_column] = df_sequence_df[
                    "Full Sequence"
                ].apply(trim_function)
            if "Duplicate" not in df_sequence_df.columns:
                df_sequence_df["Duplicate"] = df_sequence_df.duplicated(
                    subset=self.sequence_column, keep="first"
                )
            df_sequence_df["Duplicate Count"] = 1
            df_sequence_df["Duplicate Entries"] = ""
            for _, grouped_duplicates in df_sequence_df.groupby(
    ...
    def write_predefined_sequences(self) -> Job:
        ...
        This job takes a table of input sequences that can be filtered
        via self.sequence_df_filter. Susequently, these sequences can be
        trimmed to a fixed length and / or by removing given start/end
        sequences.
        ...
        outputfile = self.result_dir / f"{self.name}_predefined_sequences.tsv"
        return FileGeneratingJob(
            outputfile, self._write_predefined_sequences_callable()
        ).depends_on(self.get_dependencies())

    def count_samples_fast(
        self,
        raw_lane: Sample,
        row_order=Union[None, List[str]],
    ...
    def write_count_table_callable(
        self,
        raw_lane: Sample,
        row_order: Optional[List[str]] = None,
    ):
    ...
    def write_count_table(
        ...
        return (
            MultiFileGeneratingJob(
                [output_file, output_file2],
                self.write_count_table_callable(raw_lane, row_order),
            )
            .depends_on(
                FunctionInvariant(f"{self.name}_count_fastq", self.count_fastq)
            )
